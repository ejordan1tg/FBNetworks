...
def genAdjMatrices1(G, event_id):
    A = nx.to_numpy_array(G)

    resultsto = 'C:/Users/Elijah/FBNetworks/AdjMatrices_1/' 
    np.save(resultsto + f"adj_{event_id}.npy", A)

def genAdjMatrices2():
    source = 'C:/Users/Elijah/FBNetworks/AdjMatrices_1'
    dest = 'C:/Users/Elijah/FBNetworks/AdjMatrices_2'
    
    adjMatrices1 = glob.glob(os.path.join(source, '*.npy'))

    for file_path in adjMatrices1:
        ...
        np.save(output_path, squared_matrix)
...
def buildGraphs(complete_passes, frames):
    ...
    for i in range(len(complete_passes)):
        ...
        visited = set()
        G, event_id = connect_players(G, matching_frames, actor_loc, actorx, actory, visited, event_id)
        genAdjMatrices1(G, event_id)       

def connect_players(G, matching_frames, actor_loc, actorx, actory, visited, event_id):
    ...
    for j in range(len(matching_frames)):
        ...
        print("Added player:")
        print(player)
        connect_players(G, matching_frames, player, xj, yj, visited, event_id)
